...
@app.route('/api/clockify/preview', methods=['POST'])
@jwt_required()
def clockify_preview():
    """
    Clockify time entries'lerini rapor çıkarmadan önce önizle.
    İlk 10 satırı ve özet istatistikleri döndürür.
    """
    try:
        user_id = get_jwt_identity()
        user = mongo.db.users.find_one({'_id': ObjectId(user_id)})
        ...
        if not api_key:
            return jsonify({'error': 'Clockify API key not configured'}), 400
        ...
        report_payload = {
            ...
            "detailedFilter": {"page": 1, "pageSize": 200}  # Preview için 200 yeterli
        }
        if project_ids:
            report_payload["detailedFilter"]["projects"] = {
                "ids": project_ids, "contains": "CONTAINS"
            }
        ...
        # CSV formatına çevir (rapor ile aynı format)
        csv_rows = []
        total_seconds = 0
        ...
        for entry in time_entries:
            try:
                ...
                end_time   = datetime.fromisoformat(end_str.replace('Z', '+00:00')) if end_str \
                             else start_time + timedelta(seconds=duration_seconds)

                dur_sec = duration_seconds if duration_seconds > 0 else (end_time - start_time).total_seconds()
        ...
        total_duration_str = f"{total_h}h {total_m}m"

        # Unique değerler
        unique_users    = list({r['User']    for r in csv_rows if r['User']})
        unique_projects = list({r['Project'] for r in csv_rows if r['Project'] != 'No Project'})
        unique_clients  = list({r['Client']  for r in csv_rows if r['Client']  != 'No Client'})

        columns    = ['Project', 'Client', 'User', 'Description', 'Start Date', 'Start Time', 'End Time', 'Duration (h)', 'Billable']
        sample     = csv_rows[:10]  # İlk 10 satır
        ...
        return jsonify({
            ...
            'unique_values': {
                ...
                'Client':  unique_clients
            }
        }), 200

    except Exception as e:
        app.logger.error(f"Error in clockify_preview: {str(e)}\n{traceback.format_exc()}")
